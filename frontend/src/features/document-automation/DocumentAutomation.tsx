...
import type { BadgeProps } from "@genai/ui";
import {
  CheckSquare,
  ChevronDown,
  ChevronLeft,
...
import { DocumentLibraryScreen, DocumentPickerButton, LibraryDocument, uploadLibraryFiles } from "@/features/document-library/DocumentLibrary";
...
import { ModuleWorkspace } from "@/features/module-workspace/ModuleWorkspace";
import { WorkflowBuilder, WorkflowRunResultWindow } from "@/features/workflow-builder/WorkflowBuilder";
...
export default function DocumentAutomation() {
  ...
  async function restoreWorkspaceState() {
    ...
    if (locationMode === "workflow-result") return;
    ...
    const savedMode = locationMode;
    setMode(savedMode);
    if (!saved || saved.mode !== savedMode) return;
    try {
  ...
  return (
    <div className="document-automation-shell">
      <header className="topbar">
        ...
        <div className="topbar-action-shell" ref={topbarMenu.ref}>
          ...
          <div className={`status-strip ${topbarMenu.open ? "mobile-open" : ""}`}>
            ...
            {mode !== "home" && (
              <Button type="button" variant="secondary" size="sm" onClick={() => navigateMode("home")}>
                홈
              </Button>
            )}
            {mode === "key-info" && (
      ...
      <section className={`home-hero home-hero-workflow ${props.showMonitor ? "" : "home-hero-workflow-no-monitor"}`}>
        <div className="home-hero-copy">
          ...
          <div className="home-hero-actions">
            <Button type="button" variant="primary" size="lg" className="home-workflow-cta" onClick={props.onWorkflow}>
              <FileJson size={18} />
              워크플로우 빌더 열기
            </Button>
            <Button type="button" variant="secondary" size="lg" className="home-workflow-cta" disabled={props.pocBusy} onClick={props.onSeedBankPoc}>
              <Sparkles size={18} />
              은행 서류 데모 시작
            </Button>
          </div>
...
function HomeMonitorPanel(props: {
  ...
  return (
    <Card className="home-monitor-panel">
      <CardHeader className="home-monitor-head">
        <div>
          <p className="eyebrow">최근 실행</p>
          <CardTitle>{props.activeCount ? `진행 중인 작업 ${props.activeCount}개` : "작업 진행 현황"}</CardTitle>
        </div>
      </CardHeader>
      <CardBody>
        {props.items.length ? (
              ...
              >
                <div className="home-monitor-main">
                  <div className="home-monitor-meta">
                    ...
                    <Badge className={`home-monitor-status ${item.status}`} variant={homeMonitorBadgeVariant(item.status)}>
...
function homeMonitorBadgeVariant(status: string): BadgeProps["variant"] {
  if (status === "running") return "primary";
  if (status === "failed" || status === "completed_with_errors") return "destructive";
  if (status === "paused" || status === "interrupted" || status === "canceled") return "warning";
...
function StepPill({ label, active, done }: { label: string; active: boolean; done: boolean }) {
  return <Badge className={`step-pill ${active ? "active" : ""} ${done ? "done" : ""}`} variant={active || done ? "primary" : "outline"}>{label}</Badge>;
}

function ProviderPill({ status }: { status: SystemStatus | null }) {
  if (!status) return <Badge className="provider-pill warning" variant="warning">API 상태 알 수 없음</Badge>;
  const detail = status.vlm_model_name || (status.is_mock ? "Mock" : status.has_vlm_credentials ? "모델 준비됨" : "모델 미설정");
  return (
    <Badge className={`provider-pill ${status.is_mock ? "mock" : status.has_vlm_credentials ? "ready" : "warning"}`} variant={status.has_vlm_credentials ? "success" : "warning"}>
      VLM · {detail}
    </Badge>
  );
}
