...
- 원본 문서는 `문서 보관함`에 먼저 저장하고, 모든 모듈/워크플로우는 보관함 document id를 재사용한다.

현재 구현 기능:

- **Document Library**: 파일/폴더 업로드, 업로드 대기열, 백그라운드 변환, page image/meta 생성, ready 문서 제공
...
### 1.2 2026-05-25 구조 전환 완료 범위

| ID | 내용 |
| --- | --- |
| D1 | 홈에 `문서 보관함` 진입점을 추가하고 보관함 화면을 구현 |
| D2 | `/api/library/uploads`와 `DocumentConversionJob` 기반 백그라운드 변환 queue 추가 |
| D3 | 보관함 `ready` 문서는 즉시 실행, `queued/preprocessing` 문서는 준비되면 실행되도록 모듈/워크플로우 연결 |
| D4 | KIE, Document Classifier, Required Field Checker, Workflow Builder에 `from-documents` 실행 API 추가 |
...
| D10 | 문서 보관함 목록/아이콘 보기 전환, 넓은 문서 영역, 우측 상단 작업 알림, `Esc` 선택 해제 UX 추가 |

### 1.3 문서 보관함 기준 구조

문서 보관함은 업로드와 실행을 분리하는 공통 입력 계층이다.
...
- UI는 목록 보기와 아이콘 보기를 모두 제공한다. 작업 진행/완료 메시지는 본문 중간 상태 바가 아니라 우측 상단 transient 알림으로 표시한다.
- `Esc`는 현재 선택을 해제한다. `Command/Ctrl+A/C/X/V`와 `Delete`는 보관함 영역에서 파일 관리자식 선택/clipboard/삭제 동작으로 처리한다.

모듈/워크플로우 실행 정책:

- 보관함에서 선택한 `document_ids`로 batch/run item을 생성한다.
- `ready` 문서는 즉시 `queued/running` 실행 대상으로 들어간다.
...
- 새 UX에서는 파일/폴더 선택이 곧바로 모듈 소유 업로드가 아니라 문서 보관함 업로드가 된다. 모듈은 보관함 document id를 실행 대상으로 삼는다.
- 보관함 선택 modal은 모든 모듈에서 같은 동작을 공유한다.
...
- 문서 처리 작업 수(`WORKFLOW_MAX_WORKERS`)와 AI 동시 요청 수(`VLM_MAX_CONCURRENT_REQUESTS`)는 별도 입력으로 관리한다.

Home 문서 보관함:

- 홈 기능 카드에서 `문서 보관함`으로 진입한다.
- 파일/폴더 선택과 drag/drop을 모두 지원한다.
- 업로드 중에도 다음 파일/폴더를 업로드 대기열에 계속 추가할 수 있다.
- 폴더 트리, 상태 필터, 검색, 원본 삭제를 제공한다.
...
- 보관함 ready 문서 1개를 선택해 `/api/raw-extractions/from-document`로 실행할 수 있다.
...
- 보관함 문서 선택 버튼을 제공한다. 여러 문서 선택 시 `/api/batches/from-documents`로 실행한다.
- 변환 중인 보관함 문서는 `waiting_for_document` 상태로 batch에 등록되고, 준비되면 자동 실행된다.
...
- 보관함 문서 선택 버튼을 제공하며, 여러 문서는 `/api/classification-batches/from-documents`로 실행한다.
...
- 1개 파일은 single job, 2개 이상은 batch job으로 자동 실행한다.
- 보관함 문서 선택 버튼을 제공하며, 여러 문서는 `/api/required-field-check-batches/from-documents`로 실행한다.
...
- 파일 업로드는 먼저 문서 보관함에 저장한 뒤 `/api/workflows/{workflow_id}/runs/from-documents`로 실행한다.
- 기존 보관함 문서를 선택하면 업로드를 반복하지 않고 같은 document id를 재사용한다.
...
- workflow worker 수와 AI 동시 요청 수 분리 설정
...
- document library upload/tree/delete/conversion queue
- from-documents KIE/classifier/required/workflow 실행
